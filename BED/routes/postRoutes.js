...
postRoutes.post('/createPost',validateToken,async(req,res)=>{
    ...
    else
    {
        ...
        const post=new postModel({...postData});//creating instance
        await post.save();//savng to database
...
postRoutes.get('/likePost/:postId/:userName',async(req,res)=>{
    
    //request mae userName and postId aya hai
    let postId = req.params.postId;
    let userName = req.params.userName;

    //try,catch mae ess liyae dalae hai taki error aayae toh catch wala chal jayae nahi toh if block execute ho
    try {
